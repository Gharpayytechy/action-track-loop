    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <Tabs defaultValue="run">
        ...
        <TabsContent value="run" className="space-y-5 mt-5">
          {hydrated
            ? <GuidedRun role={role} actorId={actor.id} day={day} />
            : <Card className="p-5 text-sm text-muted-foreground">Loading today's run…</Card>}
          {needsRecovery && (
            <RecoveryCard
              role={role}
              actorId={actor.id}
        ...
        <TabsContent value="flow" className="space-y-5 mt-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 font-medium"><CheckCircle2 className="h-4 w-4 text-primary" /> {actor.name} × Impact — the day, one message at a time</div>
            {hydrated ? (
              <ImpactChat
                role={role}
                phases={phases}
