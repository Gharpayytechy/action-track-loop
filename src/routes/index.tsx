...
function LeaderHome({ actor }: { actor: Employee }) {
  const pod = useMemo(
    () => EMPLOYEES.filter((e) => e.managerId === actor.id || e.team === actor.team).filter((e) => e.id !== actor.id),
    [actor.id, actor.team]
  );
...
function HRHome({ actor }: { actor: Employee }) {
  ...
  const birthdays = EMPLOYEES.filter((e) => e.birthdayMMDD === mmdd);
  const newJoiners = EMPLOYEES.filter((e) => (e.joinedYearsAgo ?? 99) === 0);
  const lowAttendance = EMPLOYEES.filter((e) => e.attendance < 80).sort((a, b) => a.attendance - b.attendance);
  const flagged = EMPLOYEES.filter((e) => e.flags.length > 0);
  ...
  return (
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-[1300px] mx-auto">
      ...
      <PodPillars pod={EMPLOYEES} label="Org" />
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
        <div className="rounded-xl bg-card border border-border p-4">
          ...
          <div className="mt-2 font-display text-2xl font-semibold">{EMPLOYEES.length}</div>
    ...
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-[1400px] mx-auto">
      ...
      <section className="grid md:grid-cols-3 gap-4 mb-6">
        <div className="rounded-xl bg-card border border-border p-5">
          ...
          <div className="text-xs text-muted-foreground mb-3">{s.top.role} · {inr(s.top.revenueImpact)}</div>
        ...
        <div className="rounded-xl bg-card border border-border p-5">
          ...
          <div className="text-xs text-muted-foreground mb-3">{s.bottom.role} · {s.bottom.flags.length} flags</div>
        ...
        <div className="rounded-xl bg-sidebar text-sidebar-foreground border border-sidebar-border p-5">
          <div className="flex items-center gap-2 text-primary mb-3">
            <TrendingUp className="h-4 w-4" /><span className="text-xs font-mono uppercase tracking-widest">AI Directive</span>
          </div>
