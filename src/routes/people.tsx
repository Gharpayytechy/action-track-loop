...
import { EMPLOYEES, type AppRole } from "@/data/seed";
...
export const Route = createFileRoute("/people")({
  component: () => <RoleGate allow={["leadership", "hr", "leader"]}><PeoplePage /></RoleGate>,
  ...
  return (
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-[1200px] mx-auto">
      <header className="mb-5">
        ...
        <p className="text-muted-foreground text-sm mt-1">Everyone in the Arena. Find them fast.</p>
      </header>
            ...
            <div key={e.id} className="rounded-2xl bg-card border border-border p-4 flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[9px] uppercase tracking-widest font-mono px-1.5 py-0.5 rounded border ${ROLE_TONE[e.appRole]}`}>{e.appRole}</span>
                </div>
                <div className="text-xs text-muted-foreground truncate">{e.role} · {e.team}</div>
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <Link to="/score" className="text-xs text-primary hover:underline">Score {score}</Link>
                </div>
              </div>
            </div>
          );
